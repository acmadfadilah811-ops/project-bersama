/**
 * Encoder Code 128-B — logika murni, tanpa React.
 *
 * Dipisah dari BarcodeSvg.jsx supaya bisa dipakai juga oleh barcodeSheet.js
 * (pembuat lembar PNG). Modul .js polos juga berarti pemakainya tidak perlu
 * ikut menarik runtime React.
 */
...
export function encode(value) {
  ...
  sequence.forEach((code) => {
    [...PATTERNS[code]].forEach((width, index) => {
      modules.push({ width: Number(width), bar: index % 2 === 0 });
    });
  });
  return { text, modules };
}
