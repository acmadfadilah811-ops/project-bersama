...
export default function TipeTransaksiPengeluaranModal({ isOpen, onClose }) {
  const [originalAccOh, setOriginalAccOh] = useState('');
  const [originalAccOhj, setOriginalAccOhj] = useState('');
  
  const [selectedAccOh, setSelectedAccOh] = useState('');
  const [selectedAccOhj, setSelectedAccOhj] = useState('');

  if (!isOpen) return null;

  const isDirty = selectedAccOh !== originalAccOh || selectedAccOhj !== originalAccOhj;

  const handleCancel = () => {
    setSelectedAccOh(originalAccOh);
    setSelectedAccOhj(originalAccOhj);
  };

  const handleSave = () => {
    setOriginalAccOh(selectedAccOh);
    setOriginalAccOhj(selectedAccOhj);
    notify({
      type: 'success',
      title: 'Tipe Transaksi Disimpan',
      message: 'Pemetaan tipe transaksi pengeluaran berhasil disimpan.'
    });
  };
    ...
    <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center z-[9999] animate-fade-in text-xs font-semibold text-slate-700">
      <div className="bg-white rounded-xl border border-slate-205 shadow-2xl w-[720px] overflow-hidden animate-scale-up">
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-100 bg-[#F8FAFC]">
          <h3 className="text-xs font-bold text-slate-800">Tipe Transaksi Pengeluaran</h3>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody>
              ...
              <tr className="border-b border-slate-50">
                <td className="py-3 font-medium text-slate-700">Oh</td>
                <td className="py-3">
                  <select
                    value={selectedAccOh}
                    onChange={(e) => setSelectedAccOh(e.target.value)}
              ...
              {/* Row 2: Ohj */}
              <tr className="border-b border-slate-50">
                <td className="py-3 font-medium text-slate-700">Ohj</td>
                <td className="py-3">
                  <select
                    value={selectedAccOhj}
                    onChange={(e) => setSelectedAccOhj(e.target.value)}
                    className="w-full px-2.5 py-1.5 border border-slate-200 rounded-md bg-white outline-none focus:border-[#0088E8] font-medium text-slate-650 cursor-pointer shadow-3xs"
                  >
                    <option value="" disabled>Pilih</option>
                    {accountsList.map((opt) => (
