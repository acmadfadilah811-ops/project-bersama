...
export default function HutangJurnalTunggal() {
  ...
  // Supplier suggestions data matching the requested data supplier
  const suppliersList = ['Supplier A', 'Supplier B', 'Supplier C', 'PT Sinar Cemerlang', 'CV Jaya Abadi', 'PT Plastik Jaya'];
  ...
  const handleRemoveCustomJournalName = (name, e) => {
    ...
    if (selectedJournalName === name) {
      setSelectedJournalName('Pembelian');
    }
  ...
  const handleAddDraft = () => {
    const newDraft = {
      ...
      txDate,
      supplier: supplierSearch || 'Cari Supplier',
      debitAcc,
      creditAcc,
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl shadow-xl overflow-hidden animate-fade-in text-xs font-semibold text-slate-700 w-full flex flex-col h-[calc(100vh-140px)] min-h-[750px]">
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 bg-[#F8FAFC]">
        ...
        <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wider">Form Jurnal Tunggal (Hutang)</h3>
        ...
        <div className="w-[43%] border-r border-slate-100 flex flex-col relative bg-slate-50/20">
          <div className="flex-1 overflow-y-auto p-5 pb-20 space-y-4">
            ...
            <div className="space-y-1">
              ...
              >
                <Plus size={12} className={showInlineAddJournal ? 'text-slate-450' : 'text-slate-550'} />
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-3.5 py-1.5 border border-slate-200 bg-white hover:bg-slate-50 text-slate-655 rounded-lg shadow-2xs font-bold text-[10px] cursor-pointer transition-colors"
                    >
                      Batal
                    </button>
                    <button
          ...
          <div className="space-y-1 relative" ref={supplierDropdownRef}>
            <label className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
              Supplier <span className="text-[#E11D48]">*</span>
            </label>
            <div className="relative flex items-center">
              <input
                ...
                className="w-full pl-3 pr-8 py-2 border border-slate-205 rounded-lg bg-white outline-none focus:border-[#0088E8] shadow-3xs"
              />
              <button
                type="button"
                onClick={() => setShowSupplierSuggestions(!showSupplierSuggestions)}
                className="absolute right-2 text-slate-400 hover:text-slate-655 cursor-pointer p-1 rounded-md hover:bg-slate-50 transition-colors"
              >
                <ChevronDown size={14} className={`transform transition-transform ${showSupplierSuggestions ? 'rotate-180' : ''}`} />
              </button>
            </div>
            ...
            <div className="space-y-4">
              {drafts.map((draft, idx) => (
                <div key={draft.id} className="border border-slate-200 rounded-xl overflow-hidden shadow-2xs animate-scale-up bg-white">
                  ...
                  <div className="flex items-center justify-between p-3.5 bg-slate-50/70 border-b border-slate-100">
                    <div className="space-y-1 min-w-0 pr-4">
                      <div className="font-bold text-slate-800 text-[11px] leading-tight break-words">
                        {draft.journalName} - {draft.debitAcc.split(' ')[0]} Pembelian & {draft.creditAcc.split(' ')[0]} Hutang ({draft.txDate})
                      </div>
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      <button
                        ...
                        className="p-1.5 border border-slate-200 hover:border-rose-200 bg-white hover:bg-rose-50 text-slate-400 hover:text-rose-505 rounded-lg transition-colors cursor-pointer shadow-3xs"
                      >
                        <Trash2 size={14} />
                      </button>
                      <button
                        ...
                        className="p-1.5 border border-slate-200 bg-white hover:bg-slate-50 text-slate-550 rounded-lg transition-colors cursor-pointer shadow-3xs"
                      >
                  ...
                  {/* Expanded draft detailed form fields */}
                  {draft.isExpanded && (
                    <div className="p-4 bg-white grid grid-cols-2 gap-x-4 gap-y-3">
                      ...
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          onChange={(e) => handleDraftChange(draft.id, 'supplier', e.target.value)}
                          className="w-full px-2 py-1.5 border border-slate-200 rounded-md bg-white text-xs font-semibold outline-none focus:border-[#0088E8]"
                        />
                      </div>
