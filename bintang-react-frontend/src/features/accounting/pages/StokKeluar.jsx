...
export default function StokKeluar() {
  ...
  const { documents, error, loading, reload } = useAccountingStockDocuments('/stock-out-documents/', {
  ...
  } = useAccountingStockBulkActions('/stock-out-documents/', documents, reload);
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3.5 flex items-start gap-3 shadow-2xs">
        ...
        <div className="space-y-1">
          <p className="font-bold text-emerald-900 text-xs">
            Data Stok Keluar dari Inventori
          </p>
          <p className="text-emerald-700 text-[11px] font-medium leading-relaxed">
            Dokumen stok keluar ditampilkan dari Inventori; tampilan dan pengaturan Akuntansi tetap terpisah.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-100 rounded-xl shadow-xs overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs border-collapse">
            <thead className="bg-slate-50/50 border-b border-slate-100 text-slate-500 font-bold">
              <tr>
                <th className="px-5 py-3.5 w-10 text-center">
                  <input type="checkbox" aria-label="Pilih semua stok keluar" checked={allVisibleSelected} onChange={toggleAllVisible} className="accent-[#0088E8] cursor-pointer" />
                </th>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={6} className="px-5 py-12 text-center text-slate-400 font-bold">Memuat data...</td></tr>
                ...
                <tr key={document.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50">
                  ...
                  <td className="px-5 py-3 text-slate-500">{document.transfer_ke || '-'}</td>
