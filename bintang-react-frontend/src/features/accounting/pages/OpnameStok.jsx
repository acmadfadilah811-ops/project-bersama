...
import OpnameStokSettingsPopover from '../components/pos/OpnameStokSettingsPopover';
...
export default function OpnameStok() {
  ...
  const { documents, error, loading, reload } = useAccountingStockDocuments('/stock-opname-documents/', {
    search: searchTx, dateFrom, dateTo, filterStatus: 'All',
  });
  const {
    selectedIds, toggleRow, toggleAllVisible, allVisibleSelected, actionLoading, handlePost, handleBatalPost,
  } = useAccountingStockBulkActions('/stock-opname-documents/', documents, reload);
  ...
  return (
    <div className="space-y-4 animate-fade-in text-xs font-semibold text-slate-700">
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3.5 flex items-start gap-3 shadow-2xs">
        ...
        <div className="space-y-1">
          <p className="font-bold text-emerald-900 text-xs">
            Data Opname Stok dari Inventori
          </p>
          <p className="text-emerald-700 text-[11px] font-medium leading-relaxed">
            Dokumen opname ditampilkan dari Inventori; tampilan dan pengaturan Akuntansi tetap terpisah.
          </p>
        </div>
      </div>
      ...
      {/* Title */}
      <h2 className="text-base font-bold text-slate-900">Opname Stok</h2>

      {/* Action Row */}
      <div className="flex flex-wrap gap-4 items-center justify-between">
        
        {/* Left Filters - Search is FIRST, Date is SECOND */}
            ...
            <thead className="bg-slate-50/50 border-b border-slate-100 text-slate-500 font-bold">
              <tr>
                <th className="px-5 py-3.5 w-10 text-center">
                  <input type="checkbox" aria-label="Pilih semua opname stok" checked={allVisibleSelected} onChange={toggleAllVisible} className="accent-[#0088E8] cursor-pointer" />
                </th>
                <th className="px-5 py-3.5">
                  <div className="flex items-center gap-1 cursor-pointer select-none hover:text-slate-700 transition-colors">
                    <span>Tanggal</span>
