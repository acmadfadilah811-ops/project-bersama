...
# Kolom persis file PDF asli Olsera (ledger_detail_11101...pdf): Tanggal, Transaksi,
# Deskripsi, Debit, Kredit, Jumlah — TANPA kolom Pelanggan/Supplier terpisah
# (beda dari deskripsi UI awal; data tetap ada di API, cuma tidak diekspor di sini).
DETAIL_COLUMNS = [
    ("Tanggal", 12), ("Transaksi", 18), ("Deskripsi", 30),
    ("Debit", 15), ("Kredit", 15), ("Jumlah", 15),
]
...
def build_ledger_all_accounts_detail_export(accounts_with_history):
    ...
    for account, history in accounts_with_history:
        ...
        for row in history["rows"]:
            ...
            ws.cell(row=row_idx, column=6, value=float(row["debit"]))
            ws.cell(row=row_idx, column=7, value=float(row["kredit"]))
            ws.cell(row=row_idx, column=8, value=float(row["running_balance"]))
            row_idx += 1
...
def build_ledger_account_export(account, history):
    ...
    for row in history["rows"]:
        ...
        ws.cell(row=row_idx, column=4, value=float(row["debit"]))
        ws.cell(row=row_idx, column=5, value=float(row["kredit"]))
        ws.cell(row=row_idx, column=6, value=float(row["running_balance"]))
        row_idx += 1
