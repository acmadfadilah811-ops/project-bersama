...
class FonnteWebhookView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        from ..wa_logic import (
            ...
            proses_kirim_desain, get_form_order,
        ...
        # ── STEP 1b: Konfirmasi 'sesuai' utk form order yang sudah direkap ──
        # Gerbang sebelum Order benar-benar dibuat (instruksi user 2026-08-15) —
        # dicek SEBELUM dispatch lain supaya balasan "sesuai" tidak nyasar ke AI
        # fallback. Kalau tidak ada data pending, biarkan jatuh ke dispatch normal.
        parsed_pending = pending_order_form.get(sender)
        if parsed_pending and cocok_konfirmasi_sesuai(message):
            pending_order_form.discard(sender)
        ...
        # ── STEP 1c: Jawaban PILIHAN PRODUK setelah info kategori/harga ──
        ...
        if sender in menunggu_pilihan_produk:
            ...
            jawaban_diluar_alur = cek_harga(message, nama_pelanggan, nomor=sender)
            if jawaban_diluar_alur:
                simpan_ke_memori(sender, "assistant", jawaban_diluar_alur, nama_pelanggan)
                return self._kirim_balas(jawaban_diluar_alur)

            if _pesan_konfirmasi_tanpa_produk(message):
            ...
            kategori_slug = menunggu_pilihan_produk.get(sender)
            menunggu_pilihan_produk.discard(sender)
            info_konteks = get_pricelist_kategori(kategori_slug) if kategori_slug else ''
            nama_produk_dipilih = ekstrak_produk_pilihan(message, info_konteks)
            menunggu_status_desain.set(sender, nama_produk_dipilih)
            jawaban = (
        ...
        # ── STEP 1d: Jawaban status desain utk produk yang sudah dipilih ──
        produk_dipilih = menunggu_status_desain.get(sender)
        if produk_dipilih:
            # NB: nama variabel sengaja BUKAN `status` — itu nama modul
            # `rest_framework.status` yang dipakai di method ini juga (mis.
            # status.HTTP_500_...); assignment ke `status` di sini akan
            # membuatnya local variable utk SELURUH fungsi & merusak baris
            # lain yang masih mengacu ke modulnya (UnboundLocalError).
            status_desain = cocok_status_desain(message)
            ...
            menunggu_status_desain.discard(sender)
            ...
            if status_desain == 'belum':
                ...
                # status 'sudah' atau tidak jelas (& bukan pertanyaan lain
                # yang bisa dijawab) — tetap kirim form order, jangan macet
                # nunggu jawaban yes/no yang lebih spesifik lagi.
            ...
            if is_form_order or is_form_desain:
                ...
                try:
                    if is_form_desain:
                        ...
                        # di blok STEP 1b (cek pending_order_form) di atas.
                        parsed = self._parse_form_order(sender, nama_pelanggan, detail_bersih)
                        pending_order_form.set(sender, parsed)
        ...
        # ── STEP 3b: Form pembatalan pesanan ──────────────────────
        if not jawaban:
            ...
            if all(penanda in p_kecil for penanda in FORM_PEMBATALAN_PENANDA):
                jawaban, admin_notify = proses_form_pembatalan(message, nama_pelanggan)
                if admin_notify:
                    ...
                    if manager_user and manager_user.no_hp:
                        ...
                        try:
                            whatsapp_client.send_text_message(mgr_wa, admin_notify)
                        except Exception as e:
                            logger.warning(f"Gagal notifikasi admin (Fonnte, pembatalan): {e}")

        # ── STEP 4: Cek tanya harga (jawab dari Product nyata, TANPA form) ──
        if not jawaban:
            jawaban = cek_harga(message, nama_pelanggan, nomor=sender)

        # ── STEP 5: Aturan awal (sapaan, katalog nyata, minta form) ───
        if not jawaban:
            jawaban = cek_rules_awal(message, sender, nama_pelanggan)

        # ── STEP 6: AI (KoboiLLM) — percakapan kontekstual/di luar keyword.
        # Diletakkan SEBELUM FAQ database supaya jawabannya tidak kaku (AI
        # mengarahkan pelanggan pakai keyword yang tepat kalau soal harga/
        # produk/order, dan menjawab natural untuk percakapan lain).
        if not jawaban:
            jawaban = tanya_ai_finishing(sender, nama_pelanggan)

        # ── STEP 7: FAQ dari database — jaring pengaman terakhir kalau AI
        # gagal total (mis. API down) dan sempat mengembalikan jawaban kosong.
        if not jawaban:
            jawaban = cek_database_faq(message, nama_pelanggan)

        from ..wa_logic import TOMBOL_MARKER
        if jawaban.startswith(TOMBOL_MARKER):
            # Fonnte tidak didukung kirim tombol di sini — fallback ke daftar
            # angka manual supaya menunya tetap kepakai.
            jawaban = jawaban[len(TOMBOL_MARKER):] + (
                "\n\n1. 📋 Order\n2. 💰 Tanya Produk\n3. 📦 Cek Status\n"
    ...
    def _parse_form_order(self, nomor, nama_kontak, detail):
        """
        Parse teks form WA jadi struktur data — TANPA simpan ke DB. Dipakai utk
        bangun rekap & menunggu konfirmasi 'sesuai' dari pelanggan sebelum Order
        benar-benar dibuat (lihat _buat_order_dari_data & pending_order_form,
        instruksi user 2026-08-15 — sebelumnya Order langsung dibuat begitu form
        terdeteksi, tanpa kesempatan pelanggan koreksi). Support multi-item:
        tiap blok 'Item N' jadi 1 item terpisah. Raise ValueError (pesan field
        kurang) kalau ada Bahan/Finishing wajib yang kosong.
        """
        ...
        for blok in blok_items:
            ...
            if kurang:
                field_kurang_list.append((nomor_item, jenis_produk, kurang))
    ...
    def _buat_order_dari_data(self, parsed):
        """Simpan hasil _parse_form_order() ke DB (Order+OrderItem+JobBoard) —
        dipanggil setelah pelanggan konfirmasi 'sesuai' atas rekap. Logic sama
        persis dengan _simpan_order_dari_form() versi lama (langsung create begitu
        form terdeteksi), cuma sumbernya dict yang sudah diparse & divalidasi
        sebelumnya, bukan parse ulang dari teks mentah."""
        nomor = parsed['nomor']
        ...
        with transaction.atomic():
            ...
            order = Order.objects.create(
                ...
                sumber='wa',
                catatan_pelanggan=parsed['raw_detail'][:500],
            )
    ...
    def _bangun_balasan_konfirmasi_order(self, order_id, panggilan, is_desain_ready):
        ...
        ketik 'sesuai') — isinya sama dengan yang dulu langsung dikirim begitu
        form terdeteksi, cuma sekarang dipanggil belakangan lewat
        _buat_order_dari_data() di gerbang konfirmasi 'sesuai'."""
...
class BaseWhatsAppWebhookView(APIView):
    ...
    def _parse_form_order(self, nomor, nama_kontak, detail):
        """Parse teks form WA jadi struktur data — TANPA simpan ke DB (lihat
        catatan lengkap di FonnteWebhookView._parse_form_order, logic sama;
        beda cuma di _buat_order_dari_data di bawah: status_global 'draft'
